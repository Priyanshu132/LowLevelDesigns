...
public class OddStrategy extends ParkingStrategy {


    public OddStrategy(Map<Integer, Floor> floorMap, Map<Integer, Slot> slotMap) {
    ...
    public Slot getSlotId(Integer floor, VehicleType type) {
        if(Objects.isNull(floor)){
            ...
            for(Slot slot : slots)
                if(slot.isFree() && slot.getId() % 2 == 1 && slot.getType().equals(type))
                    return slot;
            return null;
        }else{
            ...
            return floorMap.get(floor).isAnySlotFree(type)
                    ? sorted.stream().filter(slot -> slot.isFree() && slot.getId() % 2 == 1 && slot.getType().equals(type)).findFirst().get()
                    : getSlotId(null,type);
        }    }
}
